...
def upload_project_pdf(
    ...
    # Lightweight per-process rate limit. Production multi-worker deployments
    # should replace this with a shared limiter.
    import time
    now = time.monotonic()
    ...
    try:
        ...
        fd, temp_path = tempfile.mkstemp(prefix="paimana_pdf_", suffix=suffix)
        os.close(fd)
        ...
        if not _validate_pdf_signature(temp_path):
            raise HTTPException(415, detail="The uploaded file is not a valid PDF document.")
        ...
        try:
            ...
            raise HTTPException(400, detail=f"The PDF could not be opened: {exc}")

        if page_count == 0:
            raise HTTPException(400, detail="The uploaded PDF contains no pages.")
        if page_count > PDF_MAX_PAGES:
            raise HTTPException(413, detail=f"PDF exceeds the {PDF_MAX_PAGES}-page limit.")

        try:
            records, pages, extraction_errors = _extract_pdf_with_timeout(temp_path)
        except FutureTimeoutError:
            raise HTTPException(408, detail="PDF parsing timed out.")
        except Exception as exc:
            raise HTTPException(400, detail=f"PDF extraction failed: {exc}")

        if not records and not extraction_errors:
            raise HTTPException(422, detail="No project records were extracted from the PDF.")
        ...
        try:
            ...
            db.rollback()
            raise HTTPException(500, detail=f"Database ingestion failed: {exc}")
...
def create_project(
    ...
    return {
        ...
        "message": "Project created successfully" if outcome["project_created"] else "Project updated successfully",
    }
