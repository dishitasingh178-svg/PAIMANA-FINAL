...
from api.routers import projects, predictions, dashboard, analytics, alerts
...
# 2. Routers FIRST (prevents static mounts from hijacking API endpoints)
...
# Optional routers (included safely if files exist)
try:
...
# 3. Static Assets (CSS and JS directories)
...
async def serve_index():
    ...
    return JSONResponse({"message": "PAIMANA Backend Online"})
...
async def general_exception_handler(request: Request, exc: Exception):
    return JSONResponse(
        status_code=500,
        content={"success": False, "error": "INTERNAL_SERVER_ERROR", "message": "An unexpected server error occurred."}
    )